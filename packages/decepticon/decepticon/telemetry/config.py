...
def resolve_config(env: dict[str, str] | None = None) -> TelemetryConfig:
    ...
    e = env if env is not None else dict(os.environ)
    # Precedence: DO_NOT_TRACK / opt-out / benchmark force off; then an env
    # override; then the persisted opt-in choice; else off.
    #
    # BENCHMARK_MODE is force-off because benchmark prompts are generated by the
    # harness, not written by a user: shipping them would swamp the corpus with
    # CTF traffic that is indistinguishable from real engagements downstream.
    if _truthy(e.get("DO_NOT_TRACK")) or _truthy(e.get("BENCHMARK_MODE")) or is_opted_out(e):
